...
public:
	...
	void rotate() {
		...
		for (int i = 0; i < points3D.size(); i++) {
			...
			float tempX2 = tempX;
			float tempY2 = tempY * cos(cam.angleY * degToRad) - tempZ * sin(cam.angleY * degToRad);
			float tempZ2 = tempY * sin(cam.angleY * degToRad) + tempZ * cos(cam.angleY * degToRad);
			points3DRotated.emplace_back(point(tempX2, tempY2, tempZ2));
		}
	}
